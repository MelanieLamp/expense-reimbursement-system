...
public class EmployeeServiceTests {
    ...
    void login_employee() {
        Employee testEmployee1 = new Employee(0, "Mel",
                "Lamp", "user", "pass");
        Assertions.assertNotNull(Mockito.when(employeeService.login(Mockito.anyString(),
                Mockito.anyString())).thenReturn(testEmployee1));
    }
    @Test
    @Order(2)
    void submit_expense(){
        Expense testExpense2 = new Expense(0, 0, 0,
                335.00, new Date(System.currentTimeMillis()), "approved", "", null);
        Assertions.assertNotNull(Mockito.when(employeeService.submitExpense(Mockito.anyInt(),
                Mockito.anyObject())).thenReturn(testExpense2));
    }
    @Test
    @Order(3)
    void get_expense_by_employee(){
        List<Expense> expList = new ArrayList<>();
        Assertions.assertNotNull(Mockito.when(employeeService.getExpensesByEmployee(Mockito.anyObject())).thenReturn(expList));
    }
}
